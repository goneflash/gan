...
tiny_generator_train_loss = tf.keras.metrics.Mean('tiny_generator_train_loss',
                                                  dtype=tf.float32)
tiny_generator_test_loss = tf.keras.metrics.Mean('tiny_generator_test_loss',
                                                 dtype=tf.float32)
...
@tf.function
def distill_train_step(input_image, tiny_generator, original_generator,
                       distill_optimizer):
...
@tf.function
def distill_test_step(input_image, tiny_generator, original_generator):
    original_output = original_generator(input_image, training=False)
...
def distill_loop(train_dataset,
                 test_dataset,
                 tiny_generator,
                 original_generator,
                 distill_optimizer,
    ...
    for epoch in range(epochs):
        ...
        for image_x in train_dataset:
            distill_train_step(image_x, tiny_generator, original_generator,
                               distill_optimizer)
        ...
        with tiny_generator_train_summary_writer.as_default():
            tf.summary.scalar('tiny_generator_loss',
                              tiny_generator_train_loss.result(),
                              step=epoch)
        ...
        for image_x in test_dataset:
            distill_test_step(image_x, tiny_generator, original_generator)
