...
generator_g_loss_train = tf.keras.metrics.Mean('generator_g_loss_train',
                                               dtype=tf.float32)
generator_f_loss_train = tf.keras.metrics.Mean('generator_f_loss_train',
                                               dtype=tf.float32)
cycle_loss_x_train = tf.keras.metrics.Mean('cycle_loss_x_train',
                                           dtype=tf.float32)
cycle_loss_y_train = tf.keras.metrics.Mean('cycle_loss_y_train',
                                           dtype=tf.float32)
identity_loss_x_train = tf.keras.metrics.Mean('identity_loss_x_train',
                                              dtype=tf.float32)
identity_loss_y_train = tf.keras.metrics.Mean('identity_loss_y_train',
                                              dtype=tf.float32)
...
generator_g_loss_test = tf.keras.metrics.Mean('generator_g_loss_test',
                                              dtype=tf.float32)
generator_f_loss_test = tf.keras.metrics.Mean('generator_f_loss_test',
                                              dtype=tf.float32)
cycle_loss_x_test = tf.keras.metrics.Mean('cycle_loss_x_test',
                                          dtype=tf.float32)
cycle_loss_y_test = tf.keras.metrics.Mean('cycle_loss_y_test',
                                          dtype=tf.float32)
identity_loss_x_test = tf.keras.metrics.Mean('identity_loss_x_test',
                                             dtype=tf.float32)
identity_loss_y_test = tf.keras.metrics.Mean('identity_loss_y_test',
                                             dtype=tf.float32)
discriminator_y_test_loss = tf.keras.metrics.Mean('discriminator_y_test_loss',
                                                  dtype=tf.float32)
discriminator_x_test_loss = tf.keras.metrics.Mean('discriminator_x_test_loss',
                                                  dtype=tf.float32)
...
@tf.function
def train_step(real_x, real_y, generator_g, generator_f, discriminator_x,
               discriminator_y, generator_g_optimizer, generator_f_optimizer,
...
@tf.function
def test_step(real_x, real_y, generator_g, generator_f, discriminator_x,
              discriminator_y):
    ...
    for epoch in range(epochs):
        ...
        for image_x, image_y in tf.data.Dataset.zip((train_x, train_y)):
            train_step(image_x, image_y, generator_g, generator_f,
                       discriminator_x, discriminator_y, generator_g_optimizer,
                       generator_f_optimizer, discriminator_x_optimizer,
                       discriminator_y_optimizer)
        ...
        with generator_f_train_summary_writer.as_default():
            tf.summary.scalar('generator_total_loss',
                              generator_f_train_total_loss.result(),
                              step=epoch)
            tf.summary.scalar('generator_loss',
                              generator_f_loss_train.result(),
                              step=epoch)
            tf.summary.scalar('cycle_loss',
                              cycle_loss_x_train.result(),
                              step=epoch)
        ...
        with generator_g_train_summary_writer.as_default():
            tf.summary.scalar('generator_total_loss',
                              generator_g_train_total_loss.result(),
                              step=epoch)
            tf.summary.scalar('generator_loss',
                              generator_g_loss_train.result(),
                              step=epoch)
            tf.summary.scalar('cycle_loss',
                              cycle_loss_y_train.result(),
                              step=epoch)
            tf.summary.scalar('identity_loss',
        ...
        with discriminator_y_train_summary_writer.as_default():
            tf.summary.scalar('discriminator_loss',
                              discriminator_y_train_loss.result(),
                              step=epoch)
        with discriminator_x_train_summary_writer.as_default():
            tf.summary.scalar('discriminator_loss',
                              discriminator_x_train_loss.result(),
                              step=epoch)
        ...
        for image_x, image_y in tf.data.Dataset.zip((test_x, test_y)):
            test_step(image_x, image_y, generator_g, generator_f,
        ...
        with generator_f_test_summary_writer.as_default():
            tf.summary.scalar('generator_total_loss',
                              generator_f_test_total_loss.result(),
                              step=epoch)
            tf.summary.scalar('generator_loss',
                              generator_f_loss_test.result(),
                              step=epoch)
            tf.summary.scalar('cycle_loss',
                              cycle_loss_x_test.result(),
                              step=epoch)
            tf.summary.scalar('identity_loss',
        ...
        with generator_g_test_summary_writer.as_default():
            tf.summary.scalar('generator_total_loss',
                              generator_g_test_total_loss.result(),
                              step=epoch)
            tf.summary.scalar('generator_loss',
                              generator_g_loss_test.result(),
                              step=epoch)
            tf.summary.scalar('cycle_loss',
                              cycle_loss_y_test.result(),
                              step=epoch)
            tf.summary.scalar('identity_loss',
        ...
        with discriminator_y_test_summary_writer.as_default():
            tf.summary.scalar('discriminator_loss',
                              discriminator_y_test_loss.result(),
                              step=epoch)
        with discriminator_x_test_summary_writer.as_default():
            tf.summary.scalar('discriminator_loss',
                              discriminator_x_test_loss.result(),
                              step=epoch)
